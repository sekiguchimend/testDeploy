...
interface ConversionOptions {
  ...
  fileName?: string;
  designInfo?: DesignInfo; // デザイン情報を追加
}

// デザイン情報のインターフェース（既存のものを使用）
...
export async function convertCorrectedDocumentToFile(
  ...
  switch (format) {
    case 'pdf':
      return await convertToPdf(fullHtmlContent, fileName);
    
    case 'docx':
      return await convertToDocx(cleanedElement, fileName);
    
    case 'txt':
      return convertToTxt(cleanedElement, fileName);
    
    case 'html':
      return convertToHtml(fullHtmlContent, fileName);
    
    case 'markdown':
      return convertToMarkdown(cleanedElement, fileName);
...
// PDF変換
async function convertToPdf(htmlContent: string, fileName: string): Promise<Blob> {
  return new Promise((resolve, reject) => {
    ...
    try {
      html2pdf().from(htmlContent).outputPdf('blob').then((blob: Blob) => {
        resolve(blob);
      }).catch(reject);
    } catch (error) {
      reject(error);
...
// DOCX変換
async function convertToDocx(correctedElement: HTMLElement, fileName: string): Promise<Blob> {
  const extractTextAndType = (element: Element): { text: string; type: HeadingLevel }[] => {
    const result: { text: string; type: HeadingLevel }[] = [];
    
    const processElement = (el: Element) => {
      const text = el.textContent?.trim() || '';
      let type: HeadingLevel = HeadingLevel.NORMAL;
  ...
  const extractedContent = extractTextAndType(correctedElement);
...
// テキストファイル変換
function convertToTxt(correctedElement: HTMLElement, fileName: string): Blob {
  const cleanedElement = correctedElement.cloneNode(true) as HTMLElement;
  cleanedElement.querySelectorAll('.diff-added, .diff-removed').forEach(el => {
    el.outerHTML = el.textContent || '';
  });
...
// HTML変換
function convertToHtml(htmlContent: string, fileName: string): Blob {
  return new Blob([htmlContent], { type: 'text/html;charset=utf-8' });
}

// Markdown変換
function convertToMarkdown(correctedElement: HTMLElement, fileName: string): Blob {
  const cleanedElement = correctedElement.cloneNode(true) as HTMLElement;
  cleanedElement.querySelectorAll('.diff-added, .diff-removed').forEach(el => {
    el.outerHTML = el.textContent || '';
  });
