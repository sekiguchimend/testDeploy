  ...
  while (attempts < maxRetries) {
    try {
      ...
      if (extractDesign) {
        ...
        prompt = `
        ${customPromptSection}
        あなたは文書分析と添削のエキスパートです。以下の職務経歴書を詳細に分析し、指定されたタスクを実行してください。
【重要: 添削の原則】
...
(添削した職務要約の内容)
■職務経歴①
◎0000年0月 ～0000年0月　会社名　雇用形態　年収
▪事業内容
(添削した事業内容)
【会社設立日】0000年0月0日【従業員数】0～0人
【業務内容】
・(添削した業務内容1)
・(添削した業務内容2)
・(添削した業務内容3)
...
jsonコピー{
  "correctedText": "添削後のテキスト全文をここに入れてください。必ず前述の職務経歴書の形式を厳密に守ってください。",
...
出力は必ず有効なJSON形式で行ってください。マークダウンのコードブロック（
json）や装飾は使用しないでください。
        【添削対象テキスト】
        ${text}
        `;
